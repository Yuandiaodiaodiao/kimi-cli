 ...
 * Extended tool registry with hook integration and wire event emission.
 */
  ...
  async handle(toolCall: ToolCall): Promise<ToolResult> {
    ...
    if (this.hookEngine?.hasHooksFor("PreToolUse")) {
      const hookResults = await this.hookEngine.trigger("PreToolUse", {
        matcherValue: name,
        inputData: { tool_name: name, tool_input: args },
      });
    ...
    if (this.hookEngine) {
      ...
      if (this.hookEngine.hasHooksFor(hookEvent as any)) {
        await this.hookEngine.trigger(hookEvent as any, {
          matcherValue: name,
          inputData: {
            tool_name: name,
            tool_input: args,
